...
Redis가 빠른 이유를 설명할 때 단순히 "I/O가 없다"로 들리면 모호하다. 백엔드 서버와 Redis 서버가 별도 프로세스인 이상 네트워크 I/O는 여전히 존재하는 것 아닌가?

**답변**: 맞다. 네트워크 I/O는 Redis든 PostgreSQL이든 동일하게 존재한다. "I/O가 없어서 빠르다"는 정확히 말하면 **디스크 I/O가 없다**는 뜻이다. 차이는 요청이 도착한 다음이다.
...
### Q3. Redis에서 "키가 있는데 값이 없는 상태"가 가능한가?
...
### Q5. SET을 다시 하면 TTL이 왜 -1로 초기화되는가?
...
### Q6. Redis Hash와 비밀번호 해시(Hash)는 어떻게 다른가?
...
### Q7. Lua Script는 누가 실행하는가? Python? Redis?

분산 락 해제에서 `eval()`로 Lua Script를 보내는데, 이 스크립트를 실제로 실행하는 주체는 Python인가 Redis인가?

**답변**: Python이 아니라 **Redis 서버가 직접** 실행한다.
...
### Q8. SCAN 커서 번호가 왜 28 같은 불규칙한 숫자인가?
...
### Q9. String(JSON) vs Hash — Hash가 만능 아닌가? JSON이 유리한 경우는?
...
### Q10. Redis Stack은 언제 쓰는 건가? 기본 Redis와 Trade-off는?
...
### Q11. BLPOP이 싱글 스레드인데 서버를 30초 블로킹하는 거 아닌가?
...
### Q12. `recent:user:1`에서 콜론으로 구분된 건 뭔가?
